...
def surfaces_peintes(plateau, nb_joueurs):
    ...
    for lig in range(get_nb_lignes(plateau)):
        for col in range(get_nb_colonnes(plateau)):
            couleur=case.get_couleur(get_case(plateau,(lig,col)))
            if couleur!=' ':
                res[couleur]+=1
    return res
...
def directions_possibles(plateau,pos):
    ...
    #{'N':' ','E':'A'}
    res=dict()
    for direction in INC_DIRECTION.keys():
        if direction=='N':
            if case.est_mur(get_case(plateau,(pos[0]-1,pos[1])))==False and pos[0]-1>=0:
                    res['N']=case.get_couleur(get_case(plateau,(pos[0]-1,pos[1])))

        if direction=='S':
            if case.est_mur(get_case(plateau,(pos[0]+1,pos[1])))==False and pos[0]+1<get_nb_lignes(plateau):
                 res['S']=case.get_couleur(get_case(plateau,(pos[0]+1,pos[1])))
                   
        if direction=='O':
            if case.est_mur(get_case(plateau,(pos[0],pos[1]-1)))==False and pos[1]-1>=0:
                    res['O']=case.get_couleur(get_case(plateau,(pos[0],pos[1]-1)))
        if direction=='E':
            if pos[1]+1<get_nb_colonnes(plateau) and case.est_mur(get_case(plateau,(pos[0],pos[1]+1)))==False:
               res['E']=case.get_couleur(get_case(plateau,(pos[0],pos[1]+1)))
...
def nb_joueurs_direction(plateau, pos, direction, distance_max):
    ...
    res_nb_joueurs=0
    pos_tir=pos
    res_nb_joueurs+=len(case.get_joueurs(get_case(plateau, pos)))
    for cran in range(distance_max):
        if direction=='N':
            pos_tir=(pos_tir[0]-1,pos_tir[1])
            if pos_tir[0]<0:
                return res_nb_joueurs
            if case.est_mur(get_case(plateau,pos_tir)):
                return res_nb_joueurs
            res_nb_joueurs+=len(case.get_joueurs(get_case(plateau,pos_tir)))
        if direction=='S':
            pos_tir=(pos_tir[0]+1,pos_tir[1])
            if pos_tir[0]>=get_nb_lignes(plateau):
                return res_nb_joueurs
            if case.est_mur(get_case(plateau,pos_tir)):
                return res_nb_joueurs
            res_nb_joueurs+=len(case.get_joueurs(get_case(plateau,pos_tir)))
        if direction=='O':
            pos_tir=(pos_tir[0],pos_tir[1]-1)
            if pos_tir[1]<0:
                return res_nb_joueurs
            if case.est_mur(get_case(plateau,pos_tir)):
                return res_nb_joueurs
        ...
        if direction=='E':
            pos_tir=(pos_tir[0],pos_tir[1]+1)
            if pos_tir[1]>=get_nb_colonnes(plateau):
                return res_nb_joueurs
            if case.est_mur(get_case(plateau,pos_tir)):
                return res_nb_joueurs
            res_nb_joueurs+=len(case.get_joueurs(get_case(plateau,pos_tir)))
...
def distances_objets_joueurs(plateau, pos, distance_max):
    ...
    """
    res = dict()
    coordonées_a_soccuper =[(pos, 0)]
    dejavisites={pos}
    while len(coordonées_a_soccuper)>0:
        position_actu, dist =coordonées_a_soccuper.pop(0)
        case_actu= get_case(plateau, position_actu)
        obj=case.get_objet(case_actu)
        joueurs =case.get_joueurs(case_actu)
        if (obj !=const.AUCUN or len(joueurs)> 0):
            if dist not in res:
                res[dist]=set()
            if obj!=const.AUCUN:
                res[dist].add(obj)
            if len(joueurs)>0:
                res[dist]=res[dist].union(joueurs)
        if dist<distance_max:
            for coordonées_directive in INC_DIRECTION.values():
                voisin_pos= (position_actu[0]+coordonées_directive[0],position_actu[1]+coordonées_directive[1])
                if verification_dans_map(voisin_pos, plateau):
                    if voisin_pos not in dejavisites:
                        case_voisine = get_case(plateau, voisin_pos)
                        if not case.est_mur(case_voisine):
                            dejavisites.add(voisin_pos)
                            coordonées_a_soccuper.append((voisin_pos, dist+1))

    return res


def verification_dans_map(coordonnées,plateau):
    if (0<=coordonnées[0]< get_nb_lignes(plateau) and 0<= coordonnées[1]< get_nb_colonnes(plateau)):
        return True
    return False
