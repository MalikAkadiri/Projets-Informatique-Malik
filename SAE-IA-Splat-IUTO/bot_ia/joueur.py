...
def Joueur(couleur, nom, reserve, surface, points, position, objet, duree_objet):
    ...
    """
    return {"couleur": couleur,"nom": nom,"reserve": reserve,"surface": surface,"points": points,"position": position,
            "objet": objet,
...
def joueur_from_str(description):
    ...
    """
    différents_element=description.split(";")
    return Joueur(différents_element[0],différents_element[8],int(différents_element[1]),int(différents_element[2]),
                  int(différents_element[3]),(int(différents_element[6]),int(différents_element[7])),
                  int(différents_element[4]),int(différents_element[5]))
...
def modifie_reserve(joueur, quantite):
    ...
    nv_reserve=joueur["reserve"]+quantite
    if nv_reserve>const.CAPACITE_RESERVOIR:
        nv_reserve=const.CAPACITE_RESERVOIR
    joueur["reserve"]=nv_reserve
    return nv_reserve
################################################################################
...
def classement_joueurs(liste_joueurs,critere):
    ...
    """
    if critere=="points":
        return sorted(liste_joueurs,key=lambda joueur:get_points(joueur),reverse=True)
    elif critere=="surface":
        return sorted(liste_joueurs,key=lambda joueur:(get_surface(joueur),get_reserve(joueur)),reverse=True)